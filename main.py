...
#Consulta 3
#La segunda película con mayor score para una plataforma determinada, 
...
async def get_second_score(plataforma):
    if plataforma == "amazon":
        ...
        #Ahora filtramos por tipo de contenido (pelicula)
        df_filtro = df_filtro[df_filtro["duration_type"] == "min"]
        #Ordenamos de forma descendente según el score y con ordén alfabético para el título
        df_filtro = df_filtro.sort_values(["score", "title"], ascending=[False, True])
        #Seleccionamos la SEGUNDA película
        pelicula = df_filtro["title"].iloc[1]
        return {"plataforma": plataforma, "pelicula": str(pelicula)}
